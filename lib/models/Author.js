...
class Author {
  ...
  static async insert({ author_name, author_dob, author_pob_pop }) {
    const { rows } = await pool.query(
      'INSERT INTO authors (author_name, author_dob, author_pob_pop) VALUES ($1, $2, $3) RETURNING *;',
      [author_name, author_dob, author_pob_pop]
    );
    console.log(rows[0]);
    return new Author(rows[0]);
  }
